...
def issues(request, project_id):
    if request.method == "GET":
        ...
        project_total_user = [(request.blog.project.creator_id, request.blog.project.creator.username,)]
...
def issues_record(request, project_id, issues_id):
    ...
    if request.method == "GET":
        reply_list = models.IssuesReply.objects.filter(issues_id=issues_id, issues__project=request.blog.project)
        # 将queryset转换为json格式
        data_list = []
...
def issues_change(request, project_id, issues_id):
    ...
    if name in ['issues_type', 'module', 'parent', 'assign']:
        ...
        if not value:
            ...
            if name == 'assign':
                # 是否是项目创建者
                if value == str(request.blog.project.creator_id):
                    instance = request.blog.project.creator
                else:
    ...
    if name == "attention":
        ...
        if not value:
            ...
            # 获取当前项目的所有成员
            user_dict = {str(request.blog.project.creator_id): request.blog.project.creator.username}
...
def invite_url(request, project_id):
    ...
    if form.is_valid():
        ...
        random_invite_code = uid(request.blog.user.mobile_phone)
        form.instance.project = request.blog.project
...
def invite_join(request, code):
    ...
    if invite_object.project.creator == request.blog.user:
        return render(request, 'invite_join.html', {'error': '创建者无需再加入项目'})

    exists = models.ProjectUser.objects.filter(project=invite_object.project, user=request.blog.user).exists()
    if exists:
        return render(request, 'invite_join.html', {'error': '已加入项目无需再加入'})

    # ####### 问题1： 最多允许的成员(要进入的项目的创建者的限制）#######
    # max_member = request.blog.price_policy.project_member # 当前登录用户他限制
    ...
    models.ProjectUser.objects.create(user=request.blog.user, project=invite_object.project)
